...
\begin{enumerate}[itemsep=3pt]
\item \textbf{生产环境 + 精度优先}: 使用 \textbf{全量微调 (Full FT)}，每国 1.1GB 独立部署。
\item \textbf{快速部署 + 接近 FT 精度}: 使用 \textbf{LoRA (独立, r=16)}，3MB 适配器，精度损失 $<$0.05。
\item \textbf{多国服务 + 省空间}: 使用 \textbf{Multi-LoRA (r=8)}，
1.12GB 覆盖 8 国，热切换。但需注意 Clean Recall 较低 (易误报)。
\item \textbf{快速原型验证}: 使用 \textbf{Adapter}，训练最快但精度最低。
\end{enumerate}
